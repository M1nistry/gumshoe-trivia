...
namespace POETrivia
{
    public class SQLite
    {
        ...
        public DataTable QueryQuestions(int categoryId)
        {
            var questionTable = new DataTable();
            questionTable.Columns.Add("question_id");
            questionTable.Columns.Add("question_question");
            ...
            using (var cnn = new SQLiteConnection(_connection))
            {
                ...
                using (var cmd = new SQLiteCommand(queryQuestions, cnn))
                {
                    ...
                    using (var rdr = cmd.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            questionTable.Rows.Add(rdr.GetInt32(0), rdr.GetString(1), rdr.GetInt32(2));
                        }
                    }
                }
            }
        ...
        public List<String> QueryAnswers(int questionId)
        {
            var answerList = new List<string>();
            using (var cnn = new SQLiteConnection(_connection))
            {
                const string queryAnswers = @"SELECT answer_value FROM Answers WHERE answers_question_id=@id;";
                cnn.Open();
                using (var cmd = new SQLiteCommand(queryAnswers, cnn))
                {
                    ...
                    using (var rdr = cmd.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            answerList.Add(rdr.GetString(0));
                        }
                    }
                }
            }
        ...
        public bool UpdateAnswer(string answer, string before, int questionId)
        {
            using (var cnn = new SQLiteConnection(_connection))
            {
